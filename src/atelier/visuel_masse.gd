...
## Un sac ouvert, avec son niveau de poudre. Le niveau est le seul retour que le
## joueur ait sur le versage — aucun chiffre, il lit sa propre graduation.
func _dessiner_sachet(couleur: Color) -> void:
	var demi: float = rayon * 0.72
	var haut: float = -rayon
	var bas: float = rayon
	var corps: PackedVector2Array = PackedVector2Array([
		Vector2(-demi, haut),
		Vector2(-demi * 1.15, bas * 0.65),
		Vector2(-demi * 0.72, bas),
		Vector2(demi * 0.72, bas),
		Vector2(demi * 1.15, bas * 0.65),
		Vector2(demi, haut),
	])
	draw_colored_polygon(corps, FOND)
	if remplissage > 0.0:
		# La poudre monte dans le sac. Rien de plus : une masse qui monte suffit.
		var niveau: float = lerpf(bas, haut * 0.85, clampf(remplissage, 0.0, 1.0))
		draw_colored_polygon(
			PackedVector2Array([
				Vector2(-demi * 1.1, niveau),
				Vector2(demi * 1.1, niveau),
				Vector2(demi * 0.78, bas),
				Vector2(-demi * 0.78, bas),
			]),
			PRETE.darkened(0.25)
		)
	draw_polyline(corps, couleur, 1.0)
	# Le col ouvert, par ou tombe la poudre.
	draw_line(Vector2(-demi, haut), Vector2(demi, haut), TRAIT_EFFACE, 0.6)
	# L'anneau de suspension.
	draw_arc(Vector2(0.0, haut), 3.0, PI, TAU, 16, couleur, 1.0)
